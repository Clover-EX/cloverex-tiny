...
class UserServiceImpl(private val connection: SqlConnection) : UserService {
  ...
  override suspend fun register(user: JsonObject): JsonObject {
    ...
    return try {
      ...
      connection
        .preparedQuery("insert into public.user(username, password) values ($1, $2)")
        .executeAwait(Tuple.of(username, hashpw))
      tx.commitAwait()
      JsonObject().put("status", 201)
    } catch (e: Exception) {
      JsonObject().put("status", 409).put("message", "username already exist")
    }
  }
}
